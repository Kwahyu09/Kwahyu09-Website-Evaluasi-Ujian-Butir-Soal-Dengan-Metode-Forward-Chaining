                    ...
                    <form action="/ujian/{{ $post->slug }}/update" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="nama_ujian"
                                ...
                                required="required" value="{{ old('nama_ujian', $post->nama_ujian) }}">
                            @error('nama_ujian')
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <select class="custom-select" id="modul_id" name="modul_id">
                                        @foreach ($modul as $m)
                                            <option value="{{ $m->id }}"
                                                {{ old('modul_id', $post->modul_id) == $m->id ? 'selected' : '' }}>
                                                {{ $m->nama_modul }}</option>
                                        @endforeach
                                    </select>
                                    @error('modul_id')
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <select class="custom-select" id="kelas_id" name="kelas_id">
                                        @foreach ($kelas as $k)
                                            <option value="{{ $k->id }}"
                                                {{ old('kelas_id', $post->kelas_id) == $k->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" name="tanggal"
                                        ...
                                        required="required" value="{{ old('tanggal', $post->tanggal) }}">
                                    @error('grup_soal')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <input type="time" name="waktu_mulai"
                                        ...
                                        required="required" value="{{ old('waktu_mulai', $post->waktu_mulai) }}">
                                    @error('waktu_mulai')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="time" name="waktu_selesai"
                                        ...
                                        id="waktu_selesai" required="required"
    ...
    <script>
        ...
        nama_ujian.addEventListener('change', function() {
            fetch('/ujian/create/checkSlug?nama_ujian=' + encodeURIComponent(nama_ujian.value))
                ...
                .then(data => {
                    slug.value = data.slug; // Update nilai slug setelah menerima respons dari server
                    console.log('Slug fetched:', data.slug);
                })
                .catch(error => console.error('Error fetching slug:', error));
        });
        ...
        function loadGrupSoal(modulId, selectedGrupSoalId = null) {
                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        data.forEach(grup => {
                            ...
                            if (selectedGrupSoalId && selectedGrupSoalId == grup.id) {
                                option.selected =
                                true; // Menandai opsi sebagai dipilih jika id sama dengan selectedGrupSoalId
                            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const initialGrupSoalId = {{ $post->grup_soal_id }};
            if (initialModulId) {
                loadGrupSoal(initialModulId, initialGrupSoalId);
            }
        });
    </script>
@endsection